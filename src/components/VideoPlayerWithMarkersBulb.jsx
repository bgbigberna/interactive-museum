...
const VideoPlayerWithMarkersBulb = () => {
  const videoRef = useRef(null);
  const [markers, setMarkers] = useState(() => {
    const saved = localStorage.getItem('videoBookmarksBulb');
  ...
  useEffect(() => {
    if (markers.length === 0) {
      localStorage.removeItem('videoBookmarksBulb');
    } else {
      localStorage.setItem('videoBookmarksBulb', JSON.stringify(markers));
    }
  }, [markers]);
